...
namespace Zhangyue.iReader.EBK3
{
    public class Ebk3Open
    {
        ...
        public static EBK3HeadInfoModel GetRecodeModel(byte[] allbuffer, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                switch (num3)
                {
                    ...
                    case 9:
                        model.name = Encoding.Unicode.GetString(buffer5, 0, buffer5.Length);
                        break;

                    case 10:
                        model.author = Encoding.Unicode.GetString(buffer5, 0, buffer5.Length);
                        break;

                    case 11:
                        model.subject = Encoding.Unicode.GetString(buffer5, 0, buffer5.Length);
                        break;

                    case 12:
                        model.publisher = Encoding.Unicode.GetString(buffer5, 0, buffer5.Length);
                        break;
        ...
        public int GetChapterCount()
        {
            ...
            return this.mChapterList.Count;
        }

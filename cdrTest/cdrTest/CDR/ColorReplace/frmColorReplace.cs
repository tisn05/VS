...
namespace cdrTest
{
    public partial class frmColorReplace : BaseForm
    {
        ...
        {
            Palette pal;
            pal = app.Palettes.CreateFromSelection("Selected in " + app.ActiveDocument.Name,Overwrite:true);
            foreach(Shape s in app.ActiveSelection.Shapes.FindShapes(Type: cdrShapeType.cdrBitmapShape))
            {
        ...
        {
            Palette pal;
            pal = app.Palettes.CreateFromDocument("Colors in " + app.ActiveDocument.Name, Overwrite: true);

...
Console::displayString(string output) 
{
	...
	//if we have an open socket, use it, else pump out to command line
	if(theController->HasOpenSocket())
	{
		theController->appendToResultString(output + '\n');
	}
	else
	{
...
Console::displayStringNow(string output) 
{
	...
	if(theController->HasOpenSocket())
	{
		theController->WriteLine(output + "\n");
	}
	else
	{
...
{
	CLIController * theController = CLIController::Instance();
	theController->WriteCharString((char*)msg, strlen((char*)msg));
}

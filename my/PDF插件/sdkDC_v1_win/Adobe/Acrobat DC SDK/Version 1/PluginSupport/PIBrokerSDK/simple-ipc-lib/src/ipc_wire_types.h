...
#if ACROBAT_SANDBOX
  TYPE_STDSTRING8,      // std::string.
#endif
  TYPE_STRING16,        // 16-bit string any encoding.
#if ACROBAT_SANDBOX
  TYPE_STDSTRING16,		// std::wstring.
#endif
...
#if ACROBAT_SANDBOX
  WireType(std::string v) : MultiType(ipc::TYPE_STDSTRING8) { Set(v); }
#endif

  WireType(const wchar_t* pc) : MultiType(ipc::TYPE_STRING16) { Set(pc); }

#if ACROBAT_SANDBOX
  WireType(std::wstring v) : MultiType(ipc::TYPE_STDSTRING16) { Set(v); }
#endif
...
#if ACROBAT_SANDBOX
  const std::string RecoverStdString8() const {
    if (Id() == ipc::TYPE_STDSTRING8) return store_str8; 
    else if (Id() == ipc::TYPE_NULLSTRING8) return "";
    else throw int(ipc::TYPE_STDSTRING8);
  }
#endif
...
#if ACROBAT_SANDBOX
  const std::wstring RecoverStdString16() const {
    if (Id() == ipc::TYPE_STDSTRING16) return store_str16; 
    else if (Id() == ipc::TYPE_NULLSTRING16) return L"";
    else throw int(ipc::TYPE_STDSTRING16);
  }
#endif

...
class Channel {
 public:
  static const size_t kMaxNumArgs = 20;
  ...
  size_t Receive(DispatchT* top_dispatch) {
    ...
    do {
      ...
      if ((handler.MsgId() == kMessagePrivNewTransport) &&
		...
		// ***SAFEACROBAT CHANGE***: Check NULL before calling OnMsgIn
		int msg_id = handler.MsgId();
        if(top_dispatch->MsgHandler(msg_id) != NULL) {
          retv = top_dispatch->MsgHandler(msg_id)->OnMsgIn(msg_id, this, args, np);
		} else {
          return RcErrBadMessageId;
        }
#else

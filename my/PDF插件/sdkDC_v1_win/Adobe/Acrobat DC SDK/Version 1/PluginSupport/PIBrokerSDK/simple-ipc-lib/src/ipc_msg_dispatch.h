...
#if ACROBAT_SANDBOX
  size_t DispatchImpl(const Int2Type<11>&, ChannelT* ch, const WireType* const args[]) {
    return static_cast<DerivedT*>(this)->OnMsg(ch, PC(args[0]).p0(), PC(args[1]).p1(), PC(args[2]).p2(),
                                               PC(args[3]).p3(), PC(args[4]).p4(), PC(args[5]).p5(),
                                               PC(args[6]).p6(), PC(args[7]).p7(), PC(args[8]).p8(),
                                               PC(args[9]).p9(),  PC(args[10]).p10());
  }
#endif
};
...
class MsgOut {
 protected:
  ...
  size_t SendMsg(int msg_id, ChannelT* ch, const WireType& a0, const WireType& a1, const WireType& a2,
      const WireType& a3, const WireType& a4, const WireType& a5, const WireType& a6,
      const WireType& a7, const WireType& a8, const WireType& a9, const WireType& a10)  {
    const WireType* const args[] = { &a0, &a1, &a2, &a3, &a4, &a5, &a6, &a7, &a8, &a9, &a10 };
    return ch->Send(msg_id, args, 11);
  }
#endif
...
#if ACROBAT_SANDBOX
#define IPC_MSG_P11(rt, tname)                              \
  rt p10() const {                                           \
    COMPILE_CHK(11 <= kNumParams);                          \
    return wt_->Recover##tname();                           \
  }
#endif
